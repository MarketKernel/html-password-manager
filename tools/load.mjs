 ...
 * page runs. kdbxweb is bundled in too: its UMD build hides its named exports
 * from Node's ESM loader, and esbuild's CommonJS interop does not.
 */
...
export async function load(...modules) {
  ...
  const dir = await mkdtemp(join(tmpdir(), 'hpm-test-'));
