...
/** The text in the current language: `t('entry', 'Copy {what}', { what })`. */
...
/**
 * A text with a number in it: `tn('status', '{count} entry', '{count} entries', n)`.
...
export function dateFormat(options: Intl.DateTimeFormatOptions): Intl.DateTimeFormat {
  ...
  if (!format) {
    format = new Intl.DateTimeFormat(current, options);
 ...
 *   <p data-i18n="gate">or drag a <code data-slot="file">.kdbx</code> file here</p>
 * is looked up as "or drag a {file} file here".
 *
